...
class star:
    def __repr__(self):
        output = f"Name: {self.name} uuid:{self.uuid}"
        output += "\n\t" + f"Seed: {self.seed}"
        output += "\n\t" + f"Luminosity: {self.luminosity}"
        output += "\n\t" + f"Mass: {self.mass}"
        output += "\n\t" + f"Life: {self.life}"
        output += "\n\t" + f"Age: {self.age}"
        output += "\n\t" + f"Ecosphere: {self.ecosphere}"
        return output

    def __init__(self, star_data):
        self.luminosity = float(star_data['luminosity'])
        self.uuid = star_data['id']
        self.mass = float(star_data['mass'])
        self.life = float(star_data['life'])
        self.age = float(star_data['age'])
        self.ecosphere = float(star_data['r_ecosphere'])
        self.name = star_data['name']        
        self.seed = star_data['seed']
